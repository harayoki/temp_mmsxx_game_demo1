...
#   powershell -File build-deploy.ps1 -NoLogo    コンソールのロゴを出さない
#
...
param([switch]$Local, [switch]$Obfuscate, [switch]$NoLogo)
...
if ($NoLogo) { $logoFlag = 'false' } else { $logoFlag = 'true' }
...
$check.Dispose()
"{0} ビルド: {1}" -f $buildName, (Split-Path -Leaf $zipPath)
