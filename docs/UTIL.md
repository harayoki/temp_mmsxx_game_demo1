...
## 15. 位置を丸める — `snapTo` / `snapHit` / `snapView`

MSX の BG スプライトは **8 ドット刻みでしか置けない**。
当たり判定も同じ刻みで取らないと、見えている絵とずれる。
その丸めの式が **ゲーム側とエンジン側に写しで 2 つあった** ので、
`engine/video.js` の 1 つにまとめた。

```js
import { snapTo } from './video.js';
snapTo(13, 8);   // 8   … 8 ドット刻みに丸める
snapTo(13, 0);   // 13  … 単位が 0 以下なら丸めない
snapTo(-3, 8);   // -8  … 負の値でも下方向に丸める
```

ゲームから使う口はエンジンの 2 つ。**用途で呼び分ける**。

| 呼ぶもの | 何のため | 刻み |
| --- | --- | --- |
| `mmsxx.snapView(v)` | 絵の重ね合わせ（目・王冠・炎）を本体にそろえる | 8 固定（機械の決まり） |
| `mmsxx.snapHit(v)` | 当たり判定に使う位置 | `mmsxx.hitSnap` |

```js
mmsxx.hitSnap = 8;   // 見た目に合わせる（既定。MSX ならこれ）
mmsxx.hitSnap = 0;   // 丸めない（数のまま当てる）
mmsxx.hitSnap = 16;  // 別の刻みを持つ機械
```

- **吸着するかどうかも、いくつで丸めるかも、ゲームごとに変わる。**
  8 ドット刻みを持たない機械へ移しても、宣言を 1 行変えるだけで済む
- **見た目のほうは変えられない。**あちらは機械が決めること
- 画面が無くても同じ値が出るので、**そのまま回帰試験の土台になる**

STAR FABLE では、ボスが見た目の `sx/sy` と当たりの `hx/hy` を別に持ちます。
既定ではどちらも同じ値になるので、手ざわりは変わりません。
