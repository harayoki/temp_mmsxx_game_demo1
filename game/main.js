...
if (PAD_ON) {
  touchGui = new TouchGui({
    ...
    touch: {
      ...
      idleFire: true,
      // **向きは、いま指が動いている向きをそのまま読む**('move')。
      // 触れたところからの向き('origin')だと、右へ出したぶんを戻さないと
      // 左が立たず、細かく行き来する避けゲーでは折り返しが重かった。
      // move なら折り返しはその場でつながり、**指を止めれば止まる**。
      // **?stick=origin で前の手ざわりへ戻せる**(見比べるため)
      stickMode: OPT.get('stick') === 'origin' ? 'origin' : 'move',
    },
    lang: TG_LANG,
