...
// 札そのものは `pointer-events: none` で焦点を取らない(下の絵を触れる)。
// **押せるボタンだけが指を受ける**(キーボードの無い端末のための入口)。
//
// ## 閉じかた
//
...
// **いちばん上に置く。** 札はゲームを止める相手なので、何かの下に潜ると
// 答えようがなくなる。実際、スマホの GUI の器(z-index 10)が上に乗っていて、
// **ボタンがマウスでも指でも押せなくなっていた**(キーでは答えられるので気づきにくい)
const BASE_STYLE = 'display:none;position:absolute;left:0;top:50%;z-index:9996;'
