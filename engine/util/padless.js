  ...
  return {
    ...
    update(selfX, selfY, dtMs = 1000 / 60) {
      ...
      if (state === 'steer') {
        ...
        // **仮の印は指のところ。そのまま。**
        //
        // 前方の縁へ寄せて「離したらここになる」を先に見せる案もあるが、
        // **縁は進んでいる向きと一緒に回る**ので、指を止めていても印だけが
        // 自機のまわりを滑っていく。押さえている場所に印が無いのは
        // 「行き先が勝手に動く」としか見えないので、正直さより素直さを取る。
        // 前方への丸めは、離して確定するときに一度だけ効かせる
        marker.x = fx; marker.y = fy;
        // **指のところまで来たら、そこで浮いて待つ。**
