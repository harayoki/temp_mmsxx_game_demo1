...
//
// 全部消したいときは**画面の外を叩く**(呼ぶ側が stop() を呼ぶ)。
// **画面の中に「押しても行き先にならない場所」は作らない** —
// 自機を叩いたら止まる、も試したが、いちばん押しやすいところなのに
// 押しても何も置けないのは分かりにくかった(引きずればそこへ来るので要らない)。
//
...
export function createPadless(opts = {}) {
  ...
  return {
    ...
    down(x, y, selfX, selfY) {
      // **自機を叩いて止める仕掛けは無い。**
      // 引きずればそこへ来るので、止めたいところへ置けば済む。
      // 「ここは押しても行き先にならない」場所が画面の中にあると、
      // 押したのに何も起きない、が起きる(いちばん押しやすいところなのに)。
      // 全部消したいときは**画面の外**を叩く(呼ぶ側が stop() を呼ぶ)
      //
