...
const DEFAULTS = {
   ...
   *
   * **3.2 = 真ん中でひと粘りして、終わりで一気に立つ。**
   * 不感帯(4)を出てから 10px で 0.02、14px で 0.11、18px で 0.32、
   * 24px で全速。**全開の位置は動かしていない**ので、逃げ足は今までどおり。
   * 変えたのは手前の割りふりだけで、真ん中のあたりが粘るようになる
   * (2.5 では 10px で 0.08 出ていて、置いた指の揺れで動きだしていた)。
   *
...
export class TouchControls {
  ...
  _stickMove(p) {
    ...
    // **矢印が立つのは不感帯を「出てから」。**
    // `<` にしていたころは、ふちにちょうど乗ったところで
    // 倒し量がまだ 0 なのに矢印だけ立ち、**キーの向き(強さは常に 1)へ
    // 落ちて全速で走りだしていた**(input.stick は倒し量が 0 のとき
    // キーを見る)。ふちは不感帯の側に入れて、境目を作らない
