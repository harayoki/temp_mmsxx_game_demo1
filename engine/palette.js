...
/**
 * **アンテナ(RF)でつないだときの色**を作る。
 *
 * 昔のテレビは、映像を電波に載せてから戻していた。そのあいだに
 *   - 色の信号だけ帯域が狭いので、**鮮やかさが落ちる**
 *   - 全体に白っぽく浮いて、**黒が沈みきらない**
 *   - 走査の熱で、少し**暖色に転ぶ**
 * ということが起きる。その 3 つを数字でまねる。
 * @param {number[][]} base もとにする色の並び
 */
function rfDegrade(base) {
...
// アンテナ(RF)の色は、MMSXX の色を劣化させて作る
PALETTES.rf = rfDegrade(PALETTES.tms9918);
